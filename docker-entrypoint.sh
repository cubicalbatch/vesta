...
# (or a previous root-run image) left behind. Repair ownership so the
# unprivileged runtime user can write zims/, models/, vesta.db, cache/ — then
# exec the app as that user instead of root.
mkdir -p "$DATA_DIR"
